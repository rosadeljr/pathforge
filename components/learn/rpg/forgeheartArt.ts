...
function defs() {
  return `<defs>
 <linearGradient id="sky" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#16344a"/><stop offset="55%" stop-color="#102636"/><stop offset="100%" stop-color="#0b1a26"/></linearGradient>
...
function hatVec(hat: string, a: string) { switch (hat) { case "circlet": return `<path d="M14 19 Q28 13 42 19" fill="none" stroke="#fcd34d" stroke-width="3" stroke-linecap="round"/><circle cx="28" cy="15.5" r="3" fill="#a5f3fc"/>`; case "cap": return `<path d="M12 21 Q28 3 44 21 Q40 13 28 13 Q16 13 12 21 Z" fill="${a}"/><rect x="10" y="19" width="36" height="4" rx="2" fill="#fcd34d"/>`; case "hood": return `<path d="M10 28 Q9 5 28 5 Q47 5 46 28 Q40 14 28 14 Q16 14 10 28 Z" fill="${a}"/>`; case "glasses": return `<g stroke="#16202e" stroke-width="1.2" fill="none"><circle cx="21" cy="25" r="4.5" fill="rgba(165,243,252,0.5)"/><circle cx="35" cy="25" r="4.5" fill="rgba(165,243,252,0.5)"/><path d="M25.5 25 h5"/></g>`; default: return ""; } }
function chibi(x: number, y: number, look: { accent: string; item?: string; hat?: string; acc?: string }, width: number, flip = false) {
  const a = look.accent, item = look.item || "none", hat = look.hat || "none", acc = look.acc || "none"; const sk = "#f7d6b3", hr = "#3b2a1a", t = "#fcd34d", ol = "#16202e"; const sc = width / 56, tx = x - 28 * sc, ty = y - 64 * sc; const noFace = hat === "glasses";
  const shield = acc === "shield" ? `<g transform="translate(13,44)"><path d="M0 -7 L6 -4 L6 3 Q6 8 0 10 Q-6 8 -6 3 L-6 -4 Z" fill="${a}" stroke="${ol}"/></g>` : "";
  const goggles = acc === "goggles" ? `<g stroke="${ol}" stroke-width="1.2"><circle cx="22" cy="16" r="4" fill="#a5f3fc"/><circle cx="34" cy="16" r="4" fill="#a5f3fc"/><path d="M26 16 h4"/></g>` : "";
  return `<g transform="translate(${tx},${ty}) scale(${sc}) ${flip ? `translate(56,0) scale(-1,1)` : ""}"><ellipse cx="28" cy="61" rx="14" ry="3.6" fill="#000" opacity="0.3"/><rect x="23.5" y="50" width="4" height="9" rx="2" fill="#27324a"/><rect x="28.5" y="50" width="4" height="9" rx="2" fill="#27324a"/><ellipse cx="25.5" cy="59.5" rx="3.2" ry="1.8" fill="#0f172a"/><ellipse cx="30.5" cy="59.5" rx="3.2" ry="1.8" fill="#0f172a"/><path d="M19 40 Q28 36 37 40 L35.5 52.5 Q28 55.5 20.5 52.5 Z" fill="${a}" stroke="${ol}" stroke-width="0.7"/><path d="M19 40 Q28 36 37 40 L36 46 Q28 49 20 46 Z" fill="#fff" opacity="0.12"/><rect x="20.5" y="48.5" width="15" height="2.6" rx="1.3" fill="#2a2118"/><path d="M24.5 39.5 L28 43.5 L31.5 39.5" fill="none" stroke="${t}" stroke-width="1.4" stroke-linecap="round"/><path d="M18.5 40 q-2.6 5 -1 9.5 l2.6 -0.8 q-1 -4.5 0.8 -7.7 z" fill="${a}" stroke="${ol}" stroke-width="0.6"/><path d="M37.5 40 q2.6 5 1 9.5 l-2.6 -0.8 q1 -4.5 -0.8 -7.7 z" fill="${a}" stroke="${ol}" stroke-width="0.6"/><circle cx="39" cy="46" r="2.4" fill="${sk}"/>${shield}${itemVec(item)}<circle cx="28" cy="24" r="15.5" fill="${sk}" stroke="${ol}" stroke-width="0.8"/><path d="M12.5 24 Q12 8 28 7 Q44 8 43.5 24 Q41 15 33 14 Q28 10.5 23 14 Q15 15 12.5 24 Z" fill="${hr}" stroke="${ol}" stroke-width="0.7"/>${hatVec(hat, a)}${noFace ? `<circle cx="22.5" cy="26.5" r="1.6" fill="#1f2937"/><circle cx="34" cy="26.5" r="1.6" fill="#1f2937"/>` : `<ellipse cx="22.5" cy="26" rx="2.8" ry="3.4" fill="#fff"/><ellipse cx="34" cy="26" rx="2.8" ry="3.4" fill="#fff"/><circle cx="23" cy="26.7" r="1.8" fill="#1f2937"/><circle cx="34.4" cy="26.7" r="1.8" fill="#1f2937"/><circle cx="22.3" cy="25.6" r="0.7" fill="#fff"/><circle cx="33.7" cy="25.6" r="0.7" fill="#fff"/>`}<circle cx="19.5" cy="30" r="2" fill="#fb7185" opacity="0.4"/><circle cx="37" cy="30" r="2" fill="#fb7185" opacity="0.4"/><path d="M24.5 31 Q28 33.6 31.5 31" fill="none" stroke="#9a5b3b" stroke-width="1.2" stroke-linecap="round"/></g>`;
}
function bubble(x: number, y: number, text: string, a: string) { const w = text.length * 6.8 + 28; return `<g><rect x="${x - w / 2}" y="${y - 22}" width="${w}" height="22" rx="9" fill="rgba(245,251,255,0.97)" stroke="${a}" stroke-width="1.5"/><text x="${x}" y="${y - 7}" font-size="13" font-weight="600" fill="#0f1b2a" text-anchor="middle" font-family="Inter,system-ui,sans-serif">${text}</text><polygon points="${x - 5},${y} ${x + 5},${y} ${x},${y + 6}" fill="rgba(245,251,255,0.97)"/></g>`; }
function nameplate(x: number, y: number, text: string, a: string) { const w = text.length * 6.2 + 16; return `<g><rect x="${x - w / 2}" y="${y}" width="${w}" height="17" rx="6" fill="rgba(10,22,38,0.82)" stroke="${a}"/><text x="${x}" y="${y + 12}" font-size="11" font-weight="700" fill="#eaf6ff" text-anchor="middle" font-family="Inter,system-ui,sans-serif">${text}</text></g>`; }
...
const playerSvg = (x: number, y: number, accent: string, look: Look, name: string, level: number) => `<g class="rpg-float-slow"><ellipse cx="${x}" cy="${y - 2}" rx="26" ry="9" fill="${accent}" opacity="0.22"/><ellipse cx="${x}" cy="${y - 2}" rx="26" ry="9" fill="none" stroke="${accent}" stroke-width="2.5" opacity="0.8"/><ellipse cx="${x}" cy="${y - 2}" rx="18" ry="6" fill="none" stroke="#eaffff" stroke-width="1" opacity="0.5"/>${bubble(x, y - 92, "That's me!", accent)}${chibi(x, y, { accent, item: look.item, hat: look.hat, acc: look.acc }, 80)}${nameplate(x, y + 6, `${name} · Lv ${level}`, accent)}</g>`;

export function buildTownSvg(opts: { classId: string; accent: string; name: string; level: number; maps: string }) {
  ...
  s += playerSvg(620, 680, accent, look, name, level);
  s += `<g>${parts}</g>`;
